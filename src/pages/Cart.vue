<template>
  <div>
        ...
        <div class="item-list-wrap">
          <div class="cart-item">
            ...
            <ul class="cart-item-list">
              <template v-for="item in userCartData">
                <li>
                <div class="cart-tab-1">
                  <div class="cart-item-check">
                    <a href="javascipt:;" class="checkbox-btn item-check-btn" :class="{'checked':item.checked=='1'}" @click="toggle(item)">
                  ...
                  <div class="item-quantity">
                    <div class="select-self select-self-open">
                      <div class="select-self-area">
                        <a class="input-sub" @click="reduce(item)">-</a>
                        <span class="select-ipt">{{item.productNum}}</span>
                        <a class="input-add" @click="add(item)">+</a>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        <div class="cart-foot-wrap">
          <div class="cart-foot-inner">
            <div class="cart-foot-l">
              <div class="item-all-check">
                <a href="javascipt:;" @click="toggleCheckAll">
                  <span class="checkbox-btn item-check-btn" :class="{'checked':checkFlag}">
            ...
            <div class="cart-foot-r">
              ...
              <div class="btn-wrap">
                <a class="btn btn--red" :class="{'btn--dis':checkedCount==0}" @click="checkOut">前往支付</a>
              </div>
            </div>
          </div>
        </div>
    ...
    <my-dialog :isShow="isShowDelDialog" @on-close="closeDialog('isShowDelDialog')">
      <del-cart-confirm @cancel="closeDialog('isShowDelDialog')" :iid="this.iid" :uname="this.username"></del-cart-confirm>
    </my-dialog>
  </div>
</template>
<style>
...
<script>
  ...
  import delCartConfirm from '../components/base/delCartConfirm'
    export default{
        name: 'Cart',
        components:{
          myBread:Bread,
        ...
        data(){
            return{
                ...
                //uname:this.$store.getters.username,
                userCartData:[],
                //checkFlag:false
                iid:"",
                username:""
      ...
      computed:{
        ...
        checkFlag(){
            return this.checkedCount==this.userCartData.length;
        },
        checkedCount(){
            var i=0;
            this.userCartData.forEach((item)=>{
                if(item.checked)
                {
                    i++;
                }
      ...
      methods:{
          getUserData(){
              ...
              this.$axios.post("/queryUserCart",this.$qs.stringify(data))
                .then((res)=>{
                  //console.log(res);
        ...
        //用户是否全部选中商品
        toggleCheckAll(){
            var flag=!this.checkFlag;
          this.userCartData.forEach((item)=>{
            item.checked=flag;
          })
            let data={
                uname:this.uname,
                checkAll:this.checkFlag
            };
            console.log(data.checkAll);
            this.$axios.post("/checkedAll",this.$qs.stringify(data))
              .then((res)=>{
                console.log(res);
        ...
        toggle(item){
            ...
            this.$axios.post("/checked",this.$qs.stringify(data))
              .then((res)=>{
                console.log(res);
              })
              .catch((err)=>{
        ...
        del(item){
          ...
          console.log(this.iid,this.uname);
