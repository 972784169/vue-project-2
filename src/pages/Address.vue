      ...
      <div class="container">
        <div class="checkout-addr">
          ...
          <div class="addr-list-wrap">
            <div class="addr-list">
              <ul>
                <template v-for="(item,index) in addressListFliter" >
                  <li :class="[{'check':checkIndex==index}]" @click="toggleCheck(item,index)">
                    <dl>
                      <dt>{{item.username}}</dt>
                      <dd>{{item.country}}&nbsp;{{item.province}}&nbsp;{{item.city}}</dd>
                      <dd class="address">{{item.streetname}}&nbsp;{{item.address}}</dd>
                      <dd class="tel">{{item.cellphone}}</dd>
                    </dl>
                    ...
                    <div class="addr-opration addr-set-default">
                      <a href="javascript:;" class="addr-set-default-btn" @click="setDafault(item)" v-if="item.is_default=='0'"><i>Set default</i></a>
                    </div>
                    <div class="addr-opration addr-default" v-if="item.is_default=='1'">
                      default
                    </div>
                  </li>
                </template>
                <li class="addr-new">
                  <div class="add-new-inner">
                    <i class="icon-add" @click="showDialog">
            ...
            <div class="shipping-addr-more">
              <a class="addr-more-btn up-down-btn" href="javascript:;" @click="addAddressListFliter" :class="{'open':this.limit>1}">
                更多
                <i class="i-up-down">
          ...
          <div class="next-btn-wrap">
            <a class="btn btn--m btn--red" @click="jumpOrderConfirm">Next</a>
          </div>
        </div>
      </div>
    </div>
    <my-dialog :isShow="isShowDialog" @on-close="closeDialog('isShowDialog')">
      <AddressDetail @closeDialog="closeDialog('isShowDialog')"></AddressDetail>
    </my-dialog>
    <my-dialog :isShow="isShowDelDialog" @on-close="closeDialog('isShowDelDialog')">
      <del-confirm @cancel="closeDialog('isShowDelDialog')" :aid="this.aid" :uname="this.username"></del-confirm>
    </my-dialog>
    <my-dialog :isShow="isShowAddressDialog" @on-close="closeDialog('isShowAddressDialog')">
      <address-default @cancel="closeDialog('isShowAddressDialog')" :aid="this.aid" :uname="this.username"></address-default>
    </my-dialog>
  </div>
</template>
<style>
...
<script>
  ...
  import DelConfirm from '../components/base/delConfirm'
  import addressDefault from '../components/base/addressDefault'
  export default{
    name:"Address",
    components:{
      myBread:Bread,
    ...
    methods:{
      ...
      getAddressList(){
        ...
        console.log(uname);
        this.$axios.post("/getAddressList",this.$qs.stringify({uname}))
          .then((res)=>{
      ...
      toggleCheck(item,index){
        ...
        this.selectIndex=item.aid;
      },
      //设置默认地址
      setDafault(item){
        this.aid=item.aid;
        this. username=this.uname;
        console.log(this.aid,this.username);
        this.isShowAddressDialog=true;

      },
      jumpOrderConfirm(){
          this.$router.push({path:"/OrderConfirm",query:{aid:this.selectIndex}});//跳转到用户订单确认页面，将用户选择的邮箱地址穿过去
      }
    }
  }
</script>
