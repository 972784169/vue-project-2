      ...
      <div class="checkout-order">
        ...
        <div class="item-list-wrap confirm-item-list-wrap">
          <div class="cart-item order-item">
            ...
            <ul class="cart-item-list">
              <template v-for="item in OrderConfirmList">
                <li>
        ...
        <div class="price-count-wrap">
          <div class="price-count">
            <ul>
              <li>
                <span>商品单价合计</span>
                <span>{{orderDetailPrice | currency('￥')}}</span>
              </li>
              <li>
                <span>商品合计件数:</span>
              ...
              <li>
                <span>配送费:</span>
                <span>{{orderDetailShiping | currency('￥')}}</span>
              </li>
              <li>
                <span>税收款:</span>
                <span>{{orderDetailTax | currency('￥')}}</span>
              </li>
              <li class="order-total-price">
                <span>合计:</span>
                <span>{{(orderDetailTotalPrice+orderDetailTax+orderDetailShiping) | currency('￥')}}</span>
              </li>
            </ul>
          </div>
        </div>
        ...
        <div class="order-foot-wrap">
          <div class="prev-btn-wrap">
            <a class="btn btn--m" @click="jumpAddress">退后</a>
          </div>
          <div class="next-btn-wrap">
            <button class="btn btn--m btn--red" @click="jumpOrderSuccess">创建订单</button>
          </div>
        </div>
      </div>
    </div>
...
<script>
  ...
  export default{
      ...
      data(){
          return{
            orderDetailShiping:200,
            OrderConfirmList:[]
          }
      },
      mounted(){
          this.getOrderConfirmList();
          console.log(this.$route.query.aid);//接收到用户选择的邮箱aid
      },
      computed:{
        uname(){
        ...
        orderDetailPrice(){
          let priceTotal=0;
          this.OrderConfirmList.forEach((item)=>{
            priceTotal+=parseInt(item.salePrice);
          })
          return priceTotal;
        },
        orderDetailTax(){
          let priceTax=0;
          this.OrderConfirmList.forEach((item)=>{
            priceTax+=Math.ceil((parseInt(item.salePrice*item.productNum))*0.02);
          })
          return priceTax;
        },
        orderDetailCount(){
          let totalCount=0;
          this.OrderConfirmList.forEach((item)=>{
            totalCount+=parseInt(item.productNum);
          })
          return totalCount;
        },
        orderDetailTotalPrice(){
            let priceTotal=0;
            this.OrderConfirmList.forEach((item)=>{
                priceTotal+=(item.salePrice*item.productNum);
            })
          return priceTotal;
        }
      },
      ...
      methods:{
        getOrderConfirmList(){
            ...
            this.$axios.post("/queryOrderCart",this.$qs.stringify({uname:username}))
              .then((res)=>{
                console.log(res);
                if(res.data.status==1012)
                {
                    this.OrderConfirmList=res.data.msg;
                }
                else
                {
                    alert(res.data.msg);
        ...
        jumpOrderSuccess(){
          //创建订单列表
          let nowTime = Math.round(new Date().getTime()/1000).toString();//当前时间
          let uname=this.uname;//用户名
          let num ="";
          for(var i=0;i<8;i++)
          {
              num+=Math.floor(Math.random()*10)
          }
          let orderNo=nowTime+num;
          console.log(nowTime,uname,num,orderNo,this.orderDetailTotalPrice);
          let data={
              username:uname,
              orderNo:orderNo,
              totalPrice:this.orderDetailTotalPrice+this.orderDetailTax+this.orderDetailShiping

          };
          this.$axios.post("/createOrder",this.$qs.stringify(data))
            .then((res)=>{
